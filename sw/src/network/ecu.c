...
void ecu_set_boom_lift_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2000, 7, 2, &valve->invert));
	if (e) {
...
void ecu_set_boom_fold_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2001, 7, 2, &valve->invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_boom_rotate_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2002, 7, 2, &valve->invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_drive_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2003, 7, 2, &valve->invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_steer_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2004, 7, 2, &valve->invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_left_leg_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2005, 7, 2, &valve->invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_right_leg_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2006, 7, 2, &valve->invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_implement(uint8_t implement) {
	if ((uv_canopen_sdo_write( ECU_NODE_ID, 0x2201, 0, 2, &implement))) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw180s_wheels_feed_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2013, 1, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw180s_delimbers_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2013, 3, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw180s_wheels_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2013, 2, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw180s_saw_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2013, 4, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw180s_tilt_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2013, 5, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw180s_rotator_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2013, 6, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw50_saw_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2030, 5, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_set_uw50_tilt_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( ECU_NODE_ID, 0x2031, 5, 2, &invert));
	if (e) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
