...
void ecu_set_boom_lift_params(valve_st *valve) {
	...
	e |= (uv_canopen_sdo_write( CANOPEN_SDO_CMD_WRITE_2_BYTES,
...
void ecu_set_engine_power_usage(uint16_t value) {
	if ((uv_canopen_sdo_write( CANOPEN_SDO_CMD_WRITE_2_BYTES,
...
void ecu_set_uw180s_wheels_feed_params(uint16_t speed_f,
	...
	e |= (uv_canopen_sdo_write( CANOPEN_SDO_CMD_WRITE_1_BYTE,
...
void ecu_set_uw180s_log_length(uint16_t value) {
	if (uv_canopen_sdo_write(CANOPEN_SDO_CMD_WRITE_2_BYTES, ECU_NODE_ID, 0x2010, 8, value)) {
		netdev_set_transmit_failure(&dspl.network.ecu);
	}
}
...
void ecu_save_params() {
	...
	if (uv_canopen_sdo_write(CANOPEN_SDO_CMD_WRITE_4_BYTES,
