...
void login_show(void) {
	...
	uv_uibutton_init(&this->login, "Log In", &uv_uistyles[DEFAULT_BUTTON_STYLE_INDEX], login_callb);
	...
	uv_uibutton_init(&this->add_user, "Add user",
			&uv_uistyles[DEFAULT_BUTTON_STYLE_INDEX], adduser_callb);
	...
	uv_uibutton_init(&this->delete_user, "Delete user",
			&uv_uistyles[DEFAULT_BUTTON_STYLE_INDEX], deleteuser_callb);
