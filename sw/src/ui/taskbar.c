...
static void show(taskbar_state_e state) {
	...
	if (state == TASKBAR_NO_ALERTS) {
		...
		const log_entry_def_st *def = log_get_nack_def(0);
		...
		uv_uiwindow_add(&this->taskbar, &this->icon_label, 0, 0,
				20, uv_uibb(&this->taskbar)->height, uv_uilabel_step);

		uv_uilabel_init(&this->info_label, &UI_FONT_SMALL, ALIGN_CENTER_LEFT, C(0xFFFFFF),
				C(0xFFFFFFFF), (char *) def->def);
		uv_uiwindow_add(&this->taskbar, &this->info_label, 20, 0,
				uv_uibb(&this->taskbar)->width - 20 - 30, uv_uibb(&this->taskbar)->height,
				uv_uilabel_step);

		snprintf(this->count_str, TASKBAR_COUNT_STR_LEN, "1/%u", log_get_nack_count());
		uv_uilabel_init(&this->count_label, &UI_FONT_SMALL, ALIGN_CENTER, C(0xFFFFFF),
				C(0xFFFFFFFF), this->count_str);
		uv_uiwindow_add(&this->taskbar, &this->count_label, uv_uibb(&this->taskbar)->width - 30,
				0, 30, uv_uibb(&this->taskbar)->height, uv_uilabel_step);
